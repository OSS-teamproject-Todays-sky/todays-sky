...
def get_latest_base_time():
    ...
    for hour in valid_base_hours:
        if current_hour >= hour: latest_hour = hour
        else: break
    if latest_hour == -1:
...
def get_short_term_forecast():
    ...
    params = {'serviceKey': SHORT_TERM_API_KEY, 'pageNo': '1', 'numOfRows': '1000', 'dataType': 'JSON', 'base_date': base_date, 'base_time': base_time, 'nx': str(NX), 'ny': str(NY)}
    try:
        res = requests.get(url, params=params, timeout=15); res.raise_for_status()
        return res.json()
    except requests.exceptions.RequestException as e:
        print(f"❌ 단기예보 API 요청 실패: {e}"); return None

def get_mid_term_forecast():
    now = datetime.now()
    if now.hour < 6: tmFc = (now - timedelta(days=1)).strftime('%Y%m%d1800')
    else: tmFc = now.strftime('%Y%m%d0600')
    urls = {"temp": "http://apis.data.go.kr/1360000/MidFcstInfoService/getMidTa", "land": "http://apis.data.go.kr/1360000/MidFcstInfoService/getMidLandFcst"}
    params = {'serviceKey': MID_TERM_API_KEY, 'pageNo': '1', 'numOfRows': '10', 'dataType': 'JSON', 'regId': MID_TERM_REG_ID, 'tmFc': tmFc}
    try:
        res_temp, res_land = requests.get(urls['temp'], params=params, timeout=15), requests.get(urls['land'], params=params, timeout=15)
...
def get_weather_alerts():
    ...
    params = {'serviceKey': WEATHER_ALERT_API_KEY, 'pageNo': '1', 'numOfRows': '10', 'dataType': 'JSON'}
    try:
        res = requests.get(url, params=params, timeout=15); res.raise_for_status()
        return res.json()
    except requests.exceptions.RequestException as e:
        print(f"❌ 기상특보 API 요청 실패: {e}"); return None

def process_short_term_for_current(res):
    """'현재' 날씨를 추출하되, temp_min/max가 null일 경우 시간별 기온으로 대체하는 함수"""
    if not res or res['response']['header']['resultCode'] != '00': return {}
    items = res['response']['body']['items']['item']
    now, today_str = datetime.now(), datetime.now().strftime('%Y%m%d')
    data = {}
    
    # 1. API가 제공하는 오늘의 최저/최고기온(TMN/TMX)을 우선 찾습니다.
    ...
    # 2. 만약 TMN/TMX가 없어서 null이라면 (늦은 밤), 시간별 기온(TMP)으로 대체합니다.
...
def process_mid_term_data(temp_res, land_res):
    if not temp_res or temp_res['response']['header']['resultCode'] != '00' or not land_res or land_res['response']['header']['resultCode'] != '00':
        print("중기예보 API 응답에 문제가 있습니다. (API 키, 활용신청 상태 또는 서버 상태를 확인하세요)")
        return []
    temp_item, land_item = temp_res['response']['body']['items']['item'][0], land_res['response']['body']['items']['item'][0]
    weekly_forecast = []
    for day in range(3, 8):
        date, day_of_week = (datetime.now() + timedelta(days=day)).strftime('%Y-%m-%d'), ['월', '화', '수', '목', '금', '토', '일'][(datetime.now() + timedelta(days=day)).weekday()]
        daily_data = {"date": date, "day_of_week": f"{day_of_week}요일", "temp_min": temp_item.get(f'taMin{day}'), "temp_max": temp_item.get(f'taMax{day}'), "sky_am": land_item.get(f'wf{day}Am'), "sky_pm": land_item.get(f'wf{day}Pm'), "precip_prob_am": land_item.get(f'rnSt{day}Am'), "precip_prob_pm": land_item.get(f'rnSt{day}Pm')}
        weekly_forecast.append(daily_data)
    return weekly_forecast

def process_weather_alerts(res):
    if not res or res.get('response', {}).get('header', {}).get('resultCode') != '00' or res.get('response', {}).get('body', {}).get('totalCount') == 0: return [{"alert": "현재 발효 중인 기상특보가 없습니다."}]
    items = res['response']['body']['items'].get('item', [])
    if not isinstance(items, list): items = [items]
    alerts = []
    for item in items:
        if 't1' in item and "부산" in item['t1']:
            alerts.append({"content": item['t1'].replace('o ',''), "announcement_time": item.get('tmFc', '시간정보 없음')})
